...
/* Count and fetch servers */
if (is_admin()) {
    $countResult = db_query("SELECT COUNT(*) AS total FROM devices WHERE type='server'")->get_result()->fetch_assoc();
    $total = $countResult['total'];

    $query = "SELECT * FROM devices WHERE type='server' ORDER BY name ASC LIMIT ?, ?";
    $params = [$offset, $limit];
    $types = "ii";
} else {
    $countResult = db_query("
        ...
        WHERE d.type='server' AND ud.user_id = ?", [$uid], "i"
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
    <h3 class="me-auto"><i class="bi bi-server"></i> Servers</h3>

    <div class="btn-group btn-group-sm flex-wrap">
        <!-- Wake All / Shutdown All -->
        <a data-wake-all href="<?= BASE_PATH ?>/actions/device_wake_all.php?type=server&csrf=<?= csrf_token() ?>"
           class="btn btn-success disabled"
           onclick="return confirm('Wake ALL offline servers?')">
           <i class="bi bi-arrow-up-circle"></i> Wake All
        </a>

        <a data-shutdown-all href="<?= BASE_PATH ?>/actions/device_shutdown_all.php?type=server&csrf=<?= csrf_token() ?>"
           class="btn btn-danger disabled"
           onclick="return confirm('Shutdown ALL online servers?')">
...
<input type="text" id="deviceSearch" class="form-control mb-2" placeholder="Search servers (name / IP)">
...
<form id="bulkActionForm" method="post" onsubmit="return confirmAction(event)">
                        ...
                        <td class="text-end">
                            <div class="btn-group btn-group-sm">
                                ...
                                <?php if (is_admin()): ?>
                                    ...
                                    <a href="<?= BASE_PATH ?>/actions/device_delete.php?id=<?= $row['id'] ?>&csrf=<?= csrf_token() ?>" class="btn btn-outline-danger" onclick="return confirm('Delete this server?')" title="Delete">
    ...
    <!-- Pagination -->
    <nav aria-label="Server pagination" class="mt-3">
...
function confirmAction(e) {
    ...
    return confirm(`Are you sure you want to ${action} the selected servers?`);
}
