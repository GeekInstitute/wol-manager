...
/* Load devices based on user permissions */
if (is_admin()) {
    $result = db_query("
        SELECT * FROM devices 
        WHERE type='server'
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h3><i class="bi bi-server"></i> Servers</h3>

    <!-- Wake All / Shutdown All -->
    <div class="btn-group btn-group-sm">

        <?php if (is_admin()): ?>
            <!-- Admin: Wake ALL offline devices -->
            ...
            <!-- Admin: Shutdown ALL online devices -->
        ...
        <?php else: ?>
            <!-- User: Only allowed if has permission on at least one assigned device -->
            <a data-wake-all href="<?= BASE_PATH ?>/actions/device_wake_all.php?type=server&csrf=<?= csrf_token() ?>"
                class="btn btn-success disabled">
